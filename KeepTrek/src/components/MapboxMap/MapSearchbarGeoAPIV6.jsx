...
const MapSearchBar = ({ mapInstance , onLocationSearch}) => {
            ...
            try {
                const response = await fetch(
                    `https://api.mapbox.com/search/geocode/v6/forward?q=${encodeURIComponent(
                        value
                    )}&access_token=${import.meta.env.VITE_MAPBOX_API_KEY}&autocomplete=true&limit=5`
                );
    ...
    const handleSuggestionClick = (suggestion) => {
        setQuery(suggestion.properties.name); // Set the search bar to the clicked suggestion
    ...
    const handleSearchClick = () => {
        if (selectedSuggestion && mapInstance) {
            const [lng, lat] = [selectedSuggestion.properties.coordinates.longitude, selectedSuggestion.properties.coordinates.latitude]; // Extract coordinates
            mapInstance.flyTo({ 
                center: [lng, lat], 
                zoom: 15, 
                essential: true 
